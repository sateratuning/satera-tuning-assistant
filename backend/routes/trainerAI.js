...
const isKrEvent = (v) => { const m = krMag(v); return Number.isFinite(m) && m >= KR_EVENT_THRESHOLD; };
...
function stagePathFor(entryId) { return path.join(trainerStageDir, `${entryId}.jsonl`); }
...
function buildComparisonFromMetrics(b, a) {
  const maxKrMag = (arr) => Array.isArray(arr) && arr.length ? Math.max(...arr.map(krMag).filter(Number.isFinite)) : 0;
  const countKREvents = (arr) => Array.isArray(arr) ? arr.filter(isKrEvent).length : 0;
  ...
  const before = {
    ...
    misfires: [b.misfires?.Cyl1??0,b.misfires?.Cyl2??0,b.misfires?.Cyl3??0,b.misfires?.Cyl4??0,b.misfires?.Cyl5??0,b.misfires?.Cyl6??0,b.misfires?.Cyl7??0,b.misfires?.Cyl8??0],
  };
  const after = {
    ...
    misfires: [a.misfires?.Cyl1??0,a.misfires?.Cyl2??0,a.misfires?.Cyl3??0,a.misfires?.Cyl4??0,a.misfires?.Cyl5??0,a.misfires?.Cyl6??0,a.misfires?.Cyl7??0,a.misfires?.Cyl8??0],
  };
  const deltas = {
...
function buildAliasIndex(headers) {
  ...
  return {
    ...
    wbEq: find(["wideband lambda","lambda","wb lambda","measured equivalence ratio","wb eq ratio 1 (sae)","wb eq ratio 1 (sae) (2)","wb eq ratio 5 (sae) (2)","wideband eq ratio","equivalence ratio (wb)"]),
...
function extractExtended(raw, step = 400) {
  ...
  for (let i = 0; i < dataRows.length; i += step) {
    ...
    samples.push({ t:numAt(r,idx.t), rpm:numAt(r,idx.rpm), mph:numAt(r,idx.mph), tb:numAt(r,idx.tb), pedal:numAt(r,idx.pedal), map:mapK, baro:baroK, boostPsi:Number.isFinite(mapK)&&Number.isFinite(baroK)?(mapK-baroK)*0.1450377:null, iat:tempAtF(r,idx.iat), cat:tempAtF(r,idx.cat), maf:numAt(r,idx.maf), mafPer:numAt(r,idx.mafPer), cylAir:numAt(r,idx.cylAir), load:numAt(r,idx.load), spark:numAt(r,idx.spark), kr:krMag(krRaw), injPw:numAt(r,idx.injPw), injDuty:numAt(r,idx.injDuty), frp:numAt(r,idx.frp), cmdEq:numAt(r,idx.cmdEq), wbEq:numAt(r,idx.wbEq), afr:numAt(r,idx.afr), stft1:numAt(r,idx.stft1), stft2:numAt(r,idx.stft2), ltft1:numAt(r,idx.ltft1), ltft2:numAt(r,idx.ltft2) });
  }
  if (dataRows.length) {
    const r = dataRows[dataRows.length - 1].split(",");
    const mapK = mapAtKPa(r, idx.map), baroK = mapAtKPa(r, idx.baro);
    const krRaw = numAt(r, idx.kr);
    const last = { t:numAt(r,idx.t), rpm:numAt(r,idx.rpm), mph:numAt(r,idx.mph), tb:numAt(r,idx.tb), pedal:numAt(r,idx.pedal), map:mapK, baro:baroK, boostPsi:Number.isFinite(mapK)&&Number.isFinite(baroK)?(mapK-baroK)*0.1450377:null, iat:tempAtF(r,idx.iat), cat:tempAtF(r,idx.cat), maf:numAt(r,idx.maf), mafPer:numAt(r,idx.mafPer), cylAir:numAt(r,idx.cylAir), load:numAt(r,idx.load), spark:numAt(r,idx.spark), kr:krMag(krRaw), injPw:numAt(r,idx.injPw), injDuty:numAt(r,idx.injDuty), frp:numAt(r,idx.frp), cmdEq:numAt(r,idx.cmdEq), wbEq:numAt(r,idx.wbEq), afr:numAt(r,idx.afr), stft1:numAt(r,idx.stft1), stft2:numAt(r,idx.stft2), ltft1:numAt(r,idx.ltft1), ltft2:numAt(r,idx.ltft2) };
  ...
  const rows = dataRows.map((line) => {
    ...
    return { rpm:numAt(a,idx.rpm), cyl:numAt(a,idx.cylAir), spark:numAt(a,idx.spark), kr:krMag(krRaw), iat:tempAtF(a,idx.iat), cat:tempAtF(a,idx.cat), map:mapAtKPa(a,idx.map), baro:mapAtKPa(a,idx.baro), maf:numAt(a,idx.maf), cmdEq:numAt(a,idx.cmdEq), wbEq:numAt(a,idx.wbEq), afr:numAt(a,idx.afr), ltft1:numAt(a,idx.ltft1), ltft2:numAt(a,idx.ltft2) };
  ...
  const findBin = (v, arr) => { if (!Number.isFinite(v)) return -1; for (let i=0;i<arr.length;i++) if (v<arr[i]) return i; return arr.length; };
  const mat = new Map();
  const keyOf = (ri, ai) => `${ri}:${ai}`;
  const add = (k, obj) => {
    if (!mat.has(k)) mat.set(k, { n:0, sparkSum:0, sparkMax:null, krMax:0, iatSum:0, catSum:0, mapSum:0, mafSum:0, eqErrSum:0, eqCount:0, ltftVarSum:0, ltftCount:0, boostSum:0 });
    const m = mat.get(k);
    m.n++;
    if (Number.isFinite(obj.spark)) { m.sparkSum+=obj.spark; m.sparkMax=m.sparkMax==null?obj.spark:Math.max(m.sparkMax,obj.spark); }
    ...
    if (Number.isFinite(obj.map)&&Number.isFinite(obj.baro)) { m.mapSum+=obj.map; const b=(obj.map-obj.baro)*0.1450377; if(Number.isFinite(b)) m.boostSum+=b; }
    ...
    if (Number.isFinite(measuredEq)&&Number.isFinite(obj.cmdEq)) { m.eqErrSum+=measuredEq-obj.cmdEq; m.eqCount++; }
    if (Number.isFinite(obj.ltft1)&&Number.isFinite(obj.ltft2)) { m.ltftVarSum+=Math.abs(obj.ltft1-obj.ltft2); m.ltftCount++; }
  };
  for (const r of rows) { const ri=findBin(r.rpm,rpmBins); const ai=findBin(r.cyl,airBins); if(ri<0||ai<0) continue; add(keyOf(ri,ai),r); }

  const rpmAirBins = [];
  for (let ri=0;ri<=rpmBins.length;ri++) {
    for (let ai=0;ai<=airBins.length;ai++) {
      const k=keyOf(ri,ai); const m=mat.get(k);
      if (!m||m.n<5) continue;
      rpmAirBins.push({ rpmBin:ri, airBin:ai, samples:m.n, sparkAvg:+(m.sparkSum/m.n).toFixed(2), sparkMax:m.sparkMax, krMax:m.krMax, iatAvg:+(m.iatSum/m.n).toFixed(1), catAvg:+(m.catSum/m.n).toFixed(1), mapAvg:+(m.mapSum/m.n).toFixed(1), boostAvgPsi:+((m.boostSum/m.n)||0).toFixed(2), mafAvg:+(m.mafSum/m.n).toFixed(2), eqErrAvg:m.eqCount?+(m.eqErrSum/m.eqCount).toFixed(3):null, ltftVarAvg:m.ltftCount?+(m.ltftVarSum/m.ltftCount).toFixed(2):null });
    }
  }
...
async function fetchKnowledgePack(meta) {
  ...
  for (const s of scopes) {
    const { data, error } = await supabase.from("trainer_memory").select("note, created_at").eq("scope",s).order("created_at",{ascending:false});
    if (!error&&Array.isArray(data)) notes.push(...data.map((r)=>r.note));
  }
  let history = [];
  if (meta?.vin) {
    const { data } = await supabase.from("trainer_entries").select("vehicle, aiSummary, created_at").contains("vehicle",{vin:meta.vin}).order("created_at",{ascending:false}).limit(10);
    if (Array.isArray(data)) history=data;
  } else if (meta?.model||meta?.engine) {
    const { data } = await supabase.from("trainer_entries").select("vehicle, aiSummary, created_at").order("created_at",{ascending:false}).limit(50);
    if (Array.isArray(data)) { history=data.filter((e)=>String(e?.vehicle?.model||"").toLowerCase()===String(meta?.model||"").toLowerCase()&&String(e?.vehicle?.engine||"").toLowerCase()===String(meta?.engine||"").toLowerCase()).slice(0,10); }
  }
...
router.post(
  "/trainer-ai",
  upload.fields([{ name:"beforeLog",maxCount:1 }, { name:"afterLog",maxCount:1 }, { name:"meta",maxCount:1 }]),
  async (req, res) => {
    ...
    try {
      ...
      const seedUser = {
        role: "user",
        content: JSON.stringify({ vehicle:meta, knowledgePack, comparison, samples:{ before:extBefore.samples, after:extAfter.samples }, extended:{ before:{ detected:extBefore.detected, samples:extBefore.samples, rpmAirBins:extBefore.rpmAirBins }, after:{ detected:extAfter.detected, samples:extAfter.samples, rpmAirBins:extAfter.rpmAirBins } } }, null, 2),
      };

      let aiSummary = "No summary generated.";
      try {
        const chat = await openai.chat.completions.create({ model:process.env.OPENAI_MODEL||"gpt-4o-mini", messages:[systemMsg,seedUser], temperature:0.3 });
        aiSummary = chat?.choices?.[0]?.message?.content || aiSummary;
      } catch (e) { console.warn("OpenAI summary error:", e.message); }

      chatStore.set(conversationId, { system:systemMsg, context:{ vehicle:meta, knowledgePack, comparison, samples:{ before:extBefore.samples, after:extAfter.samples }, extended:{ before:{ detected:extBefore.detected, samples:extBefore.samples, rpmAirBins:extBefore.rpmAirBins }, after:{ detected:extAfter.detected, samples:extAfter.samples, rpmAirBins:extAfter.rpmAirBins } } }, messages:[seedUser,{ role:"assistant", content:aiSummary }] });

      try {
        if (supabase) { await supabase.from("trainer_entries").insert([{ vehicle:meta, sparkChanges:[], aiSummary, created_at:new Date().toISOString() }]); }
      } catch (e) { console.warn("Supabase insert skipped:", e.message); }

      const trainer_entry_id = crypto.randomUUID();

      // ── THE FIX: include rpmAirBins in the response so the frontend ──
      // ── Spark Advisor can use log data without re-uploading         ──
      return res.json({
        conversationId,
        trainer_entry_id,
        comparison,
        ...
        meta,
        rpmAirBins: {
          before: extBefore.rpmAirBins || [],
          after:  extAfter.rpmAirBins  || [],
        },
        logs: {
...
router.post("/trainer-chat", express.json(), async (req, res) => {
  try {
    ...
    const contextReminder = { role:"system", content:`Context: ${JSON.stringify(convo.context).slice(0,12000)}` };
    const recent = convo.messages.slice(-10);
    const msgs = [convo.system, contextReminder, ...recent, { role:"user", content:message }];
    const chat = await openai.chat.completions.create({ model:process.env.OPENAI_MODEL||"gpt-4o-mini", messages:msgs, temperature:0.3 });
...
router.post("/trainer-remember", express.json(), async (req, res) => {
  ...
  } catch (e) { console.error("trainer-remember error:", e); res.status(500).json({ error:"Remember failed." }); }
});
...
router.post("/trainer-forget", express.json(), async (req, res) => {
  ...
  } catch (e) { console.error("trainer-forget error:", e); res.status(500).json({ error:"Forget failed." }); }
});
...
router.post("/trainer/save-chat", express.json(), async (req, res) => {
  try {
    ...
    if (!Array.isArray(chatPairs)||chatPairs.length===0) return res.status(400).json({ ok:false, error:"No chatPairs provided" });
    const p = stagePathFor(trainer_entry_id);
    const lines = chatPairs.map((cp) => { const msgs=[]; if(cp.system) msgs.push({role:"system",content:String(cp.system)}); msgs.push({role:"user",content:String(cp.user||"")}); msgs.push({role:"assistant",content:String(cp.assistant||"")}); return JSON.stringify({messages:msgs}); });
    fs.appendFileSync(p, lines.join("\n")+"\n", "utf8");
    const total = countLines(p);
    return res.json({ ok:true, trainer_entry_id, added:lines.length, total_examples_for_entry:total, stagePath:p });
  } catch (e) { console.error("/trainer/save-chat error:", e); return res.status(500).json({ ok:false, error:e.message }); }
});
...
router.post("/trainer/finalize", express.json(), async (req, res) => {
  try {
    ...
    if (!trainer_entry_id) return res.status(400).json({ ok:false, error:"Missing trainer_entry_id" });
    const p = stagePathFor(trainer_entry_id);
    if (!fs.existsSync(p)) return res.status(400).json({ ok:false, error:"No staged examples found for this trainer_entry_id" });
    const staged = fs.readFileSync(p,"utf8").split(/\r?\n/).filter(Boolean);
    if (appendToJsonl) fs.appendFileSync(MASTER_JSONL, staged.join("\n")+"\n", "utf8");
    fs.unlinkSync(p);
    const totalExamplesInFile = countLines(MASTER_JSONL);
    return res.json({ ok:true, trainer_entry_id, appended:staged.length, jsonlPath:MASTER_JSONL, totalExamplesInFile });
  } catch (e) { console.error("/trainer/finalize error:", e); return res.status(500).json({ ok:false, error:e.message }); }
});
...
router.post("/update-feedback", async (req, res) => {
  try {
    ...
    res.json({ success:true });
  } catch (e) { console.error("update-feedback error:", e.message); res.status(500).json({ error:"Update failed" }); }
});

router.post("/fine-tune-now", async (req, res) => {
  try {
    if (!supabase) return res.status(400).json({ error:"Supabase not configured." });
    const { data:entries, error:e1 } = await supabase.from("trainer_entries").select("*").order("created_at",{ascending:true});
    if (e1) throw new Error("Failed to fetch trainer entries");
    const fineTuneData = (entries||[]).filter((e)=>e?.aiSummary&&e?.vehicle).map((entry) => {
    ...
    const job = await openai.fineTuning.jobs.create({ training_file:file.id, model:process.env.OPENAI_FINETUNE_MODEL||"gpt-3.5-turbo-0125" });
